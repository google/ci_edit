...
class DirectoryList(app.window.Window):
  ...
  def __init__(self, program, host, inputWindow):
    ...
    app.window.SortableHeaderWindow(self.program, self.optionsRow, 'Name', 'editor',
        'filesSortAscendingByName', -41)
    label = app.window.LabelWindow(self.program, self.optionsRow, '|')
    label.setParent(self.optionsRow)
    label.color = color
    app.window.SortableHeaderWindow(self.program, self.optionsRow, 'Size ', 'editor',
        'filesSortAscendingBySize', 16)
    label = app.window.LabelWindow(self.program, self.optionsRow, '|')
    label.setParent(self.optionsRow)
    label.color = color
    app.window.SortableHeaderWindow(self.program, self.optionsRow, 'Modified ', 'editor',
...
class FileManagerWindow(app.window.Window):
  def __init__(self, program, host, inputWindow):
    ...
    if 1:
      ...
      label.setParent(self.optionsRow)
      toggle = app.window.OptionsToggle(self.program, self.optionsRow, 'dotFiles', 'editor',
          'filesShowDotFiles')
      toggle.color = colorPrefs.get('top_info')
      toggle = app.window.OptionsToggle(self.program, self.optionsRow, 'sizes', 'editor',
          'filesShowSizes')
      toggle.color = colorPrefs.get('top_info')
      toggle = app.window.OptionsToggle(self.program, self.optionsRow, 'modified', 'editor',
