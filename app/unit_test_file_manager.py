...
class FileManagerTestCases(app.fake_curses_testing.FakeCursesTestCase):
    ...
    def test_save_as(self):
        ...
        self.run_with_fake_inputs([
            ...
            self.display_check(2, 7, [u"     "]), CTRL_S,
            self.display_check(0, 0, [u" ci    Save File As"]), CTRL_Q, CTRL_Q
    ...
    def test_dir_and_path_with_cr(self):
        ...
        self.run_with_fake_inputs([
            self.display_check(0, 0, [u" ci     "]),
            self.display_check(2, 7, [u"     "]), CTRL_O,
            self.display_check(0, 0, [u" ci    Open File  "]), CTRL_A,
            ...
            self.display_find_check(u"/._ A name with ",
                                  u"cr\\r/"), KEY_ESCAPE, curses.ERR,
            self.display_check(0, 0, [u" ci     "]),
            self.display_check(2, 7, [u"     "]), CTRL_O,
            self.display_check(0, 0, [u" ci    Open File  "]), CTRL_A,
            self.write_text(self.path_to_sample(u"._ A name")), CTRL_I,
            self.display_check(5, 0, [u"example"]),
            self.display_find_check(u"/._ A name with ", u"cr\\r/"), CTRL_Q,
            CTRL_Q
        ])
    ...
    def test_open(self):
        ...
        self.run_with_fake_inputs([
            self.display_check(0, 0, [u" ci     "]),
            self.display_check(2, 7, [u"     "]), CTRL_O,
            self.display_check(0, 0, [u" ci    Open File  "]), CTRL_Q, CTRL_Q
        ])  # TODO(dschuyler): fix need for extra CTRL_Q.
    ...
    def test_open_binary_file(self):
        ...
        self.run_with_fake_inputs([
            ...
            self.display_check(2, 7, [u"     "]), CTRL_O,
            self.display_check(0, 0, [u" ci    Open File  "]), CTRL_A,
            self.write_text(self.path_to_sample(u"binary_test_file")), CTRL_J,
    ...
    def test_open_valid_unicode_file(self):
        ...
        self.run_with_fake_inputs([
            ...
            self.display_check(2, 7, [u"     "]), CTRL_O,
            self.display_check(0, 0, [u" ci    Open File  "]), CTRL_A,
            self.write_text(self.path_to_sample(u"valid_unicode")), CTRL_J,
            self.display_check(4, 7, [u"Здравствуйте"]), CTRL_Q
        ])
