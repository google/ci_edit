...
class ApplicationTestCases(app.fake_curses_testing.FakeCursesTestCase):
    ...
    def test_backspace_emoji2(self):
        self.runWithTestFile(kTestFile, [
            ...
            self.displayCheck(2, 7, [u"a⏰four"]),
            KEY_HOME,
            # First char.
            KEY_SHIFT_RIGHT,
            self.displayCheck(2, 7, [u"a⏰four"]),
            self.selectionCheck(0, 1, 0, 0, 3),
            self.displayCheckStyle(2, 7, 1, 1,
                                   self.prg.color.get(u'selected', 0)),
            self.displayCheckStyle(2, 8, 1, 10,
                                   self.prg.color.get(u'text', 0)),
            # Second char.
            KEY_SHIFT_RIGHT,
            self.displayCheck(2, 7, [u"a⏰four"]),
            self.selectionCheck(0, 3, 0, 0, 3),
            self.displayCheckStyle(2, 7, 1, 3,
                                   self.prg.color.get(u'selected', 0)),
            self.displayCheckStyle(2, 10, 1, 7,
                                   self.prg.color.get(u'text', 0)),
            # Third char.
            KEY_SHIFT_RIGHT,
            self.displayCheck(2, 7, [u"a⏰four"]),
            self.selectionCheck(0, 4, 0, 0, 3),
            self.displayCheckStyle(2, 7, 1, 4,
                                   self.prg.color.get(u'selected', 0)),
            self.displayCheckStyle(2, 11, 1, 6,
                                   self.prg.color.get(u'text', 0)),
            # Fourth char.
            KEY_SHIFT_RIGHT,
            self.displayCheck(2, 7, [u"a⏰four"]),
            self.selectionCheck(0, 5, 0, 0, 3),
            self.displayCheckStyle(2, 7, 1, 5,
                                   self.prg.color.get(u'selected', 0)),
            self.displayCheckStyle(2, 12, 1, 5,
                                   self.prg.color.get(u'text', 0)),
            CTRL_X,
            self.displayCheck(2, 7, [u"ur    "]),
            CTRL_Z,
            self.displayCheck(2, 7, [u"a⏰four"   ]),
            KEY_RIGHT,
            KEY_END,
            self.cursorCheck(2, 14),
            CTRL_J,
